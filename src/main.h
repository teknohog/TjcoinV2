...
static const int64 DUST_SOFT_LIMIT = 100000; // 0.001 TJC
/** Dust Hard Limit, ignored as wallet inputs (mininput default) */
static const int64 DUST_HARD_LIMIT = 1000;   // 0.00001 TJC mininput
...
/** Process an incoming block */
bool ProcessBlockSync(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp = NULL);
...
public:
    ...
    {
        uint256 thash;
        ocean(BEGIN(nVersion), BEGIN(thash));
        return thash;
    }
    ...
    bool ReadFromDiskSync(const CDiskBlockPos &pos)
    {
        SetNull();
...
//            return error("CBlock::ReadFromDisk() : errors in block header");
    ...
    // Apply the effects of this block (with given index) on the UTXO set represented by coins
    bool ConnectBlockSync(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &coins, bool fJustCheck=false);

    // Read a block from disk
    bool ReadFromDisk(const CBlockIndex* pindex);

    // Read a block from disk
    bool ReadFromDiskSync(const CBlockIndex* pindex);

    // Add this block to the block index, and if necessary, switch the active block chain to this
    bool AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos);
    // Add this block to the block index, and if necessary, switch the active block chain to this
    bool AddToBlockIndexSync(CValidationState &state, const CDiskBlockPos &pos);
    // Context-independent validity checks
    bool CheckBlock(CValidationState &state, bool fCheckPOW=true, bool fCheckMerkleRoot=true) const;
    // Context-independent validity checks
    bool CheckBlockSync(CValidationState &state, bool fCheckPOW=true, bool fCheckMerkleRoot=true) const;

    // Store block on disk
    // if dbp is provided, the file is known to already reside on disk
    bool AcceptBlock(CValidationState &state, CDiskBlockPos *dbp = NULL);
    bool AcceptBlockSync(CValidationState &state, CDiskBlockPos *dbp = NULL);
};
