...
uint256 hashGenesisBlock("0x3bf710c01ca6d0b2832be560abe06ef3f9617b20fb2ec3c656e369c99b549701");
static CBigNum bnProofOfWorkLimit(~uint256(0) >>26); // TjcoinV2: starting difficulty
...
const string strMessageMagic = "TjcoinV2 Signed Message:\n";
...
{
    // TjcoinV2: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::ReadFromDiskSync(const CBlockIndex* pindex)
{
    if (!ReadFromDisk(pindex->GetBlockPos()))
        return false;
    if (GetHash() != pindex->GetBlockHash())
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // TjcoinV2: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 10 * 60; // TjcoinV2: 0.6 days
static const int64 nTargetSpacing = 2.5 * 60; // TjcoinV2: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TjcoinV2: This fixes an issue where a 51% attack can change difficulty at will.
...
bool ConnectBestBlockSync(CValidationState &state) {
...
bool CBlock::ConnectBlockSync(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
   // Check it again in case a previous version let a bad block in
    if (!CheckBlock(state, !fJustCheck, !fJustCheck))
        return false;
...
bool SetBestChainSync(CValidationState &state, CBlockIndex* pindexNew)
{
    // All modifications to the coin state will be done in this cache.
    // Only when all have succeeded, we push it to pcoinsTip.
    ...
    if(vConnect.size()==1){
        BOOST_FOREACH(CBlockIndex *pindex, vConnect) {
            ...
            printf("sync readdisk connect block begin@@@@@@@@@@@@@@@@@\n");
            if (!block.ReadFromDiskSync(pindex))
                return state.Abort(_("Failed to read block"));
            int64 nStart = GetTimeMicros();
            if (!block.ConnectBlockSync(state, pindex, view)) {
                if (state.IsInvalid()) {
                    InvalidChainFound(pindexNew);
                    InvalidBlockFound(pindex);
                }
            ...
            printf(" sync readdisk connect blockend at @@@@@@@@@@@@@@@@@ %15"PRI64d"ms\n", GetTimeMillis() - nStart2);
            if (fBenchmark)
                printf("- Connect: %.2fms\n", (GetTimeMicros() - nStart) * 0.001);
...
bool CBlock::AddToBlockIndexSync(CValidationState &state, const CDiskBlockPos &pos)
{
    // Check for duplicate
    uint256 hash = GetHash();
    if (mapBlockIndex.count(hash))
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TjcoinV2: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::CheckBlockSync(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    // These are checks that are independent of context
    // that can be verified before saving an orphan block.
...
bool CBlock::AcceptBlockSync(CValidationState &state, CDiskBlockPos *dbp)
{
    // Check for duplicate
    uint256 hash = GetHash();
    if (mapBlockIndex.count(hash))
...
{
    // TjcoinV2: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool ProcessBlockSync(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    // Check for duplicate
    uint256 hash = pblock->GetHash();
    if (mapBlockIndex.count(hash))
    ...
    int64 nStart;
    nStart = GetTimeMillis();
    printf("sync first check begin-----------\n");
    // Preliminary checks
    if (!pblock->CheckBlock(state))
        return error("ProcessBlock() : CheckBlock FAILED");
     printf(" sync first check end at----------- %15"PRI64d"ms\n", GetTimeMillis() - nStart);
    CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint(mapBlockIndex);
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        // Extra checks to prevent "fill up memory by spamming with bogus blocks"
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 08/March/2014 Oil Slick Hints Malaysian Jet May Have Crashed at Sea";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394288277;
        block.nBits    = 0x1d3fffff;
        block.nNonce   = 53016746;
        //bits
#if 0
        ...
        loop
        {
            ocean(BEGIN(block.nVersion), BEGIN(thash));
        ...
        assert(block.hashMerkleRoot == uint256("0x32a237c30af8e111ef5384c5bc2fb41aca7780a9ba06be88fca82a51a739a6ab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TjcoinV2: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TjcoinV2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TjcoinV2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TjcoinV2Miner(CWallet *pwallet)
{
    printf("TjcoinV2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("TjcoinV2-miner");
    ...
    try { loop {
        ...
        printf("Running TjcoinV2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            loop
            {
                ...
                ocean(BEGIN(pblock->nVersion), BEGIN(thash));
    ...
    {
        printf("TjcoinV2Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TjcoinV2Miner, pwallet));
}
